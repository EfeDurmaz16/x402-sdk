...
from x402_sdk.interop import client as interop_client
from x402_sdk.interop.client import select_svm_challenge, select_svm_requirement
from x402_sdk.interop.exact import (
    MAX_MEMO_BYTES,
    MEMO_PROGRAM_ID,
...
class ExactRpcMetadataTests(unittest.TestCase):
    ...
    def test_fetch_mint_metadata_parses_account_data(self) -> None:
        ...
        with patch("x402_sdk.interop.exact.Client") as client_class:
    ...
    def test_build_exact_payment_signature_from_rpc_uses_inline_metadata(self) -> None:
        ...
        with patch("x402_sdk.interop.exact.fetch_mint_metadata") as fetch_metadata:
    ...
    def test_build_exact_payment_signature_from_rpc_fetches_missing_metadata(self) -> None:
        ...
        with patch(
            "x402_sdk.interop.exact.fetch_mint_metadata",
            return_value=MintMetadata(decimals=6, token_program=TOKEN_PROGRAM_ID),
        ) as fetch_metadata:
            with patch("x402_sdk.interop.exact.latest_blockhash", return_value=str(Hash.default())):
                header = build_exact_payment_signature_from_rpc(
                    requirement=requirement,
                    client_secret_key=client.to_json(),
                    rpc_url="http://rpc.test",
...
class ClientMainTests(unittest.TestCase):
    ...
    def test_main_pays_exact_challenge_and_emits_paid_result(self) -> None:
        ...
        with patch.dict(
            ...
            with patch(
                "x402_sdk.interop.client.urllib.request.urlopen",
                side_effect=[
                ...
                with patch(
                    "x402_sdk.interop.client.build_exact_payment_signature_from_rpc",
