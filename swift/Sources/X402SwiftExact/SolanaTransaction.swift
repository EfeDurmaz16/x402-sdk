...
public struct DefaultAssociatedTokenAddressResolver: AssociatedTokenAddressResolver {
    ...
    public func associatedTokenAddress(owner: SolanaPublicKey, mint: SolanaPublicKey, tokenProgram: SolanaPublicKey) throws -> SolanaPublicKey {
        #if canImport(CryptoKit)
        let program = try SolanaPublicKey(X402SwiftExact.associatedTokenProgram)
        ...
        throw X402SwiftExactError.invalidBase58("unable to derive associated token account")
        #else
        throw X402SwiftExactError.rpc("DefaultAssociatedTokenAddressResolver requires CryptoKit")
        #endif
    }
}
...
public struct ExactTransactionBuilder {
    ...
    public func buildPaymentHeader(for requirement: PaymentRequirement) async throws -> String {
        ...
        let payload = PaymentSignatureEnvelope(
            x402Version: X402SwiftExact.x402Version,
    ...
    public func buildTransaction(for requirement: PaymentRequirement) async throws -> Data {
        guard requirement.scheme == X402SwiftExact.exactScheme else {
            throw X402SwiftExactError.unsupportedScheme(requirement.scheme)
        }
        guard requirement.network.starts(with: "solana:") else {
            throw X402SwiftExactError.unsupportedNetwork(requirement.network)
        }
        guard let amount = UInt64(requirement.amount) else {
            throw X402SwiftExactError.invalidAmount(requirement.amount)
        }
        guard let feePayer = requirement.feePayer else {
            throw X402SwiftExactError.missingFeePayer
        }
        let recentBlockhash = try await blockhashProvider?.getLatestBlockhash()
        guard let recentBlockhash else {
            throw X402SwiftExactError.missingBlockhash
        }
        ...
        guard signature.count == 64 else {
            throw X402SwiftExactError.invalidSignatureLength(signature.count)
        }
...
private func computeUnitLimitInstruction(units: UInt32) -> TransactionInstruction {
    ...
    return TransactionInstruction(programId: try! SolanaPublicKey(X402SwiftExact.computeBudgetProgram), accounts: [], data: data)
}
...
private func memoInstruction(_ memo: String?) throws -> TransactionInstruction {
    ...
    if let memo {
        ...
        if bytes.count > X402SwiftExact.maxMemoBytes {
            throw X402SwiftExactError.memoTooLarge(bytes.count)
        }
    } else {
        bytes = Data(UUID().uuidString.replacingOccurrences(of: "-", with: "").utf8)
    }
    return TransactionInstruction(programId: try SolanaPublicKey(X402SwiftExact.memoProgram), accounts: [], data: bytes)
}
