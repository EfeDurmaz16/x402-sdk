    ...
    static func decode(_ string: String) throws -> Data {
        ...
        for byte in bytes {
            guard let value = index[byte] else {
                throw X402SwiftExactError.invalidBase58(string)
            }
            var carry = value
            var index = 0
...
public struct SolanaPublicKey: Equatable, Hashable, Codable {
    ...
    public init(_ base58: String) throws {
        ...
        guard decoded.count == 32 else {
            throw X402SwiftExactError.invalidBase58(base58)
        }
        self.bytes = decoded
    }
    ...
    public init(bytes: Data) throws {
        guard bytes.count == 32 else {
            throw X402SwiftExactError.invalidBase58(Base58.encode(bytes))
        }
        self.bytes = bytes
    }
