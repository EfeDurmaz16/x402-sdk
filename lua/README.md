...
- Add a stricter Associated Token Account PDA derivation check if the Lua package surface gains the needed PDA/off-curve primitives; the current server validates transfer destination through transaction shape plus RPC token-account mint/owner checks in the runtime smoke path.
